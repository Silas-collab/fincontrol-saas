...
export function TransactionModal({ isOpen, onClose, type, onSuccess }: TransactionModalProps) {
  const { accounts, fetchAccounts: refreshAccounts } = useAccounts();
  // @ts-ignore - hook não disponível
  const { categories, fetchCategories: refreshCategories } = useCategories();
      ...
      <div className="relative w-full max-w-lg bg-slate-900 rounded-2xl border border-slate-700/50 shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <span className={`absolute left-3 top-1/2 -translate-y-1/2 text-lg font-semibold ${colors.text}`}>
                R$
              </span>
              <input
                type="number"
    ...
    // @ts-ignore - notes not in type
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
            <input
              ...
              className="mt-1 w-4 h-4 rounded border-slate-600 bg-slate-700 text-violet-500
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Repetir automaticamente a cada período
              </p>
              ...
              {formData.isRecurring && (
                <select
                  ...
                  className="mt-2 w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`flex-1 px-4 py-2.5 rounded-lg ${colors.bg} text-white
